  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-unilever-blue/95 backdrop-blur-sm p-4"
        role="dialog"
        aria-modal="true"
      ...
      >
        <motion.div
          ...
          className="w-full max-w-sm bg-[#FAF8F5] border border-unilever-blue/20 overflow-hidden"
          dir={lang === "ur" ? "rtl" : "ltr"}
        >
          {/* Header */}
          <div className="bg-unilever-blue px-6 py-5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <ShieldCheck className="w-5 h-5 text-[#C2A878] shrink-0" aria-hidden="true" />
              <div>
                <h2 id="privacy-title" className="text-white font-sans text-lg uppercase tracking-tight leading-tight">
                  {tr.title}
                </h2>
                <p className="text-white/40 text-[9px] uppercase tracking-tight mt-0.5">{tr.tagline}</p>
              </div>
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
              {ageConfirmed === null && (
                <motion.div key="age-gate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <p className="text-[11px] font-bold uppercase tracking-tight text-unilever-blue/60 mb-3">{tr.ageGate}</p>
                  <div className="flex flex-col gap-2">
                    <button
                      id="age-yes-btn"
                      onClick={() => setAgeConfirmed(true)}
                      className="w-full py-3 border border-unilever-blue/20 text-[11px] font-bold uppercase tracking-tight text-unilever-blue hover:bg-unilever-blue hover:text-white transition-all duration-200 ease-in-out"
                    >
                      {tr.ageYes}
                    </button>
                    <button
                      ...
                      className="w-full py-3 border border-unilever-blue/10 text-[11px] font-bold uppercase tracking-tight text-unilever-blue/50 hover:text-unilever-blue transition-all duration-200 ease-in-out"
                    >
                      {tr.ageNo}
                    </button>
                  </div>
              ...
              {ageConfirmed === false && (
                ...
                >
                  <p className="text-sm text-unilever-blue/70 leading-relaxed">{tr.underAge}</p>
                  <button
                    onClick={onDecline}
                    className="mt-4 text-[10px] font-bold uppercase tracking-tight text-unilever-blue/50 hover:text-unilever-blue underline transition-all duration-200"
                  >
                    {tr.decline}
                  </button>
                </motion.div>
              ...
              {ageConfirmed === true && (
                <motion.div key="consent" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4">
                  {/* Purpose */}
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-tight text-unilever-blue/50 mb-2">{tr.purpose}</p>
                    <ul className="space-y-1.5">
                      {[tr.bullet1, tr.bullet2].map((b, i) => (
                        <li key={i} className="flex items-start gap-2 text-[11px] text-unilever-blue/70 leading-relaxed">
                  ...
                  <div className="border border-unilever-blue/10 overflow-hidden">
                    <button
                      id="partner-details-btn"
                      onClick={() => setShowDetails((v) => !v)}
                      className="w-full flex items-center justify-between px-3 py-2.5 text-[10px] font-bold uppercase tracking-tight text-unilever-blue/60 hover:text-unilever-blue transition-all duration-200"
                    ...
                    <AnimatePresence>
                      {showDetails && (
                        ...
                        >
                          <p className="text-[10.5px] text-unilever-blue/60 leading-relaxed px-3 pb-3">
                            {tr.partner}
                          </p>
                        </motion.div>
                      )}
                  ...
                  <div className="flex items-center gap-2 bg-[rgba(45,106,79,0.07)] border border-[rgba(45,106,79,0.2)] px-3 py-2">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-tight text-[#2D6A4F]">{tr.badge}</span>
                  </div>

                  {/* Legal footnote */}
                  <p className="text-[9.5px] text-unilever-blue/40 leading-relaxed">{tr.consent}</p>
                  ...
                  <a
                    ...
                    className="text-[9px] font-bold uppercase tracking-tight text-unilever-blue/40 hover:text-unilever-blue underline transition-all duration-200 -mt-2"
                  >
                    {tr.readMore}
                  </a>
                  ...
                  <div className="flex flex-col gap-2 pt-1">
                    <button
                      ...
                      className="w-full py-3.5 bg-unilever-blue text-white text-[11px] font-bold uppercase tracking-tight hover:bg-unilever-blue transition-all duration-200 ease-in-out"
                    >
                      {tr.accept}
                    </button>
                    <button
                      ...
                      onClick={onDecline}
                      className="w-full py-2.5 border border-unilever-blue/15 text-[10px] font-bold uppercase tracking-tight text-unilever-blue/50 hover:text-unilever-blue transition-all duration-200 ease-in-out"
                    >
                      {tr.decline}
                    </button>
                  </div>
